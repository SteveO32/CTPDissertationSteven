...
namespace Bam
{
    public class SurfaceParticleScript : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (m_myCar.IsMoving && !m_myCar.InMidAir && !m_myCar.CurrentlyInWater)
            {
                ...
                switch (curProperties.m_friendlyName)
                {
                    case "Grass":
                        m_particleSoundSource.clip = m_grassClip;
                        m_particleSoundSource.volume = Mathf.Lerp(m_particleSoundSource.volume, 0.2f * m_myCar.NormalisedForwardVelocity * volume, Time.deltaTime * 4);
                        ...
                        for (int i = 0; i < m_grassParticles.Length; i++)
                        {
                            ...
                            break;
                    case "Rock":
                        m_particleSoundSource.volume = Mathf.Lerp(m_particleSoundSource.volume, 0.25f * m_myCar.NormalisedForwardVelocity * volume, Time.deltaTime * 2);
        ...
        {
            ParticleSystem.MinMaxCurve emissionRate = effect.emission.rateOverTime;
            emissionRate.mode = ParticleSystemCurveMode.TwoConstants;
            emissionRate.constantMax = emissionAmount * m_myCar.ForwardVelocity * 0.035f;
            emissionRate.constantMin = emissionAmount * m_myCar.ForwardVelocity * 0.035f;
