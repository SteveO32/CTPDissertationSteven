...
namespace HDev
{
    public class HDev_CarLights : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            carscript = gameObject.GetComponent<FH.LandVehicle>();
        ...
        void Update()
        {
            ...
            else
            {
                ...
                else if(DayNightCycleScript.s_singleton)
                {
                    if (m_hazardlight1.intensity > 0)
                    {
                        DeactivateLights(m_LightType.Hazardlight);
            ...
            if (carscript.IsCurrentlyBraking())
            {
            ...
            if (carscript.IsCurrentlyReversing() == true)
            {
            ...
            if (Physics.Raycast(gameObject.transform.position, Vector3.up, 100, LayerMask.GetMask("Default")))
            {
                ...
                if (!carscript.IsCurrentlyReversing())
                {
            ...
            //If night time
            if (DayNightCycleScript.s_singleton)
            {
                if (!DayNightCycleScript.s_singleton.IsDay())
                {
                    if (m_insideTunnel == false)
