...
namespace HDev
{
    ...
    public class HDev_MeshDeformer : MonoBehaviour
    {
        ...
        List<HDev_DeformChunk> deformChunks = new List<HDev_DeformChunk>();
        ...
        void Start()
        {
            ...
            {
                //it is a car
                m_deformingMesh = GetComponent<Kojima.CarScript>().GetCarBody().transform.Find("Body").GetComponent<MeshFilter>().mesh;
            }
            else if (GetComponent<FH.LandVehicle>())
            {
                //it is a land vehicle
                m_deformingMesh = GetComponent<FH.LandVehicle>().GetCarBody().transform.Find("Body").GetComponent<MeshFilter>().mesh;
                List<Light> lightList = GetComponentsInChildren<Light>().ToList();
                for (int i = 0; i < lightList.Count; i++)
                {
                    lights.Add(lightList[i].gameObject);
                }
            }
            else
            {
        ...
        void ChunkSetUp()
        {
            if (deformChunkObjects.Count != 0)
            {
                ...
                {
                    HDev_DeformChunk newChunk = new HDev_DeformChunk();
                ...
                int chunkSize = m_currentVertices.Length / deformChunks.Count;
                int vertIndex = 0;

                //set up the chuncks
                //foireach vertex of the mesh
                foreach (Vector3 vert in m_currentVertices)
                {
        ...
        void removeFurthestChunkVertex(int _chunkIndex, float _chunkSize)
        {
            ...
            {
                if (Vector3.Distance(m_currentVertices[deformChunks[_chunkIndex].vertexIndexes[i]], convertedChunkPos) > furthestDistance)
                {
                    furthestIndex = i;
                    furthestDistance = Vector3.Distance(m_currentVertices[deformChunks[_chunkIndex].vertexIndexes[i]], convertedChunkPos);
                }
            }
            ...
            for (int i = 0; i < deformChunks.Count; i++)
            {
                ...
                {
                    if (Vector3.Distance(transform.InverseTransformPoint(deformChunks[i].chunkLocation.transform.position), m_currentVertices[vertexIndextoAdd])
                        < Vector3.Distance(transform.InverseTransformPoint(deformChunks[orderedChunks[orderIndex]].chunkLocation.transform.position), m_currentVertices[vertexIndextoAdd]))
                    {
            ...
            foreach (int chunkIndex in orderedChunks)
            {
                ...
                {
                    if (Vector3.Distance(tempChunkPos, m_currentVertices[vertexIndextoAdd]) < Vector3.Distance(tempChunkPos, m_currentVertices[vertIndex]))
                    {
                        deformChunks[chunkIndex].AddToVertexList(vertexIndextoAdd);
            ...
            if (m_fTimer <= 0)
            {
                ...
                foreach (ContactPoint Contact in col.contacts)
                {
                    ...
                    if (deformChunks[closestChunkIndex].deformation < 100)
                    {
                        ...
                        {
                            Vector3 changeValue = ((deformTarget - m_currentVertices[index]) / 25) * m_fdmgMod;

                            if (Vector3.Distance(changeValue, m_currentVertices[index]) <= 20)
                            {
                                m_currentVertices[index] += changeValue;
                            }
                        }
