...
namespace HDev
{
    public class HDev_SkidEffects : MonoBehaviour
    {
        ...
        void Awake()
        {
            m_myCar = GetComponent<FH.LandVehicle>();
        ...
        void LateUpdate()
        {
            RaycastHit[] wheelCasts = m_myCar.GetWheelRaycasts().ToArray();

            Vector3 velocity = GetComponent<Rigidbody>().velocity;

            for (int i = 0; i < m_skidSmokeParticles.Length; i++)
            {
                if (m_myCar.IsWheelSkidding(i) && m_myCar.WheelGrounded(i))
                {
